...
import * as state from "./state-manager";
 ...
 */
function generateStatusMarkdown(project: state.ProjectState): string {
  const lines: string[] = [
    "# Current Status",
    "",
  ...
  if (project.blockers.length === 0) {
    ...
    for (const blocker of project.blockers) {
      lines.push(
        `| ${blocker.description} | ${blocker.severity || "-"} | ${blocker.created_at || "-"} |`,
      );
    }
  }
  ...
  for (const [epicId, progress] of Object.entries(project.progress)) {
    ...
    const percent =
      progress.total > 0
        ...
        : 0;
    lines.push(
      `| ${epicId} | ${statusEmoji} | ${progress.completed}/${progress.total} (${percent}%) |`,
    );
  }
 ...
 */
function generateSessionMarkdown(session: state.SessionState): string {
  const lines: string[] = [
    "# Current Session",
    "",
  ...
  if (session.active_agents.length === 0) {
    ...
    for (const agent of session.active_agents) {
      lines.push(
        `| ${agent.name} | ${agent.task_id || "-"} | ${agent.started_at || "-"} |`,
      );
    }
  }
  ...
  if (session.completed_tasks.length === 0) {
    ...
    for (const task of session.completed_tasks) {
      lines.push(
        `| ${task.task_id} | ${task.tokens || "-"} | ${task.turns || "-"} | ${task.tools || "-"} | ${task.agent || "-"} | ${task.bugs_caught || 0} |`,
      );
    }
  }
  ...
  try {
    switch (command) {
      ...
      case "update-task": {
        ...
        const updates: Partial<state.Task> = {};
        if (options.status) {
          updates.status = options.status as state.Task["status"];
        }
      ...
      case "list-tasks": {
        ...
        if (options.epic) filter.epic_id = options.epic;
        if (options.status)
          filter.status = options.status as state.Task["status"];
      ...
      case "update-bug": {
        ...
        const bugUpdates: Partial<state.Bug> = {};
        if (options.status) {
          bugUpdates.status = options.status as state.Bug["status"];
        }
      ...
      case "list-bugs": {
        const bugFilter: state.BugFilter = {};
        if (options.epic) bugFilter.epic_id = options.epic;
        if (options.status)
          bugFilter.status = options.status as state.Bug["status"];
      ...
      case "record-task": {
        ...
        const taskData: state.SessionTask = {
      ...
      // Project Operations
      case "set-active": {
        const activeUpdates: Partial<state.ProjectState["active"]> = {};
        if (options.epic) activeUpdates.epic_id = options.epic;
        if (options.feature) activeUpdates.feature_id = options.feature;
        if (options.task) activeUpdates.task_id = options.task;
