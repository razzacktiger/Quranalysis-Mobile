...
Read `state/project.json` for current active epic if no scope specified.
...
If patterns found that need tracking:
- Add to `state/metrics.json` `signals.yellow` or `signals.red`
- Add to `meta/IMPROVEMENT-BACKLOG.md` if systemic
