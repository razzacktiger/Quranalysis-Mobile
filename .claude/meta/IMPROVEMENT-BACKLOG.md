...
| I-008 | Session | Workflow improvements should be done in dedicated sessions, not mid-project-task. Exception: critical blockers like session recording that must be fixed before session end. Consider: `/improve-workflow` command that starts a meta-session specifically for workflow changes. | Med | S | Open |
